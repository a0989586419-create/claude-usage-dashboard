...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .row.usage{grid-template-columns:1fr 1.3fr}
  ...
  @media(max-width:820px){
    .kpis{grid-template-columns:repeat(2,1fr)} .row.c2,.row.usage,.row.c3{grid-template-columns:1fr}
...
<div class="wrap">
  <header class="top">
    <div class="brand">
      ...
      <div>
        <h1>AI 用量監控面板</h1>
        <div class="sub">Claude Code · 用量總覽與盯盤</div>
      </div>
    </div>
  ...
  <div class="row usage">
    <div class="card block-card">
      <h3>用量佔比 <span class="pill">5 小時 / 本週</span></h3>
      <div class="desc">最該盯的兩個數字——現在這個視窗、這週各用了多少。基準＝歷史最高（自動），可在 generate.py 設方案上限。</div>
      <div id="block"></div>
    </div>
    <div class="card">
      <h3>每日用量 <span class="pill">近 30 天 · token 量</span></h3>
  ...
  <div class="row c3">
    <div class="card">
      <h3>各模型用量</h3>
      <div class="desc">你最常用哪個模型（依 token 量）。</div>
      <div id="modelChart"></div>
    </div>
    <div class="card">
      <h3>專案用量排行 <span class="pill">Top 8</span></h3>
      <div class="desc">哪個專案最吃 token。</div>
      <div id="projRank"></div>
    </div>
    <div class="card">
      <h3>Token 組成</h3>
      <div class="desc">快取讀取占比越高，代表越省。</div>
      <div id="comp"></div>
    </div>
  </div>
...
(function(){
  ...
  for(let g=0;g<=3;g++){ const y=pt+ih*g/3; const v=max*(1-g/3);
    grid+=`<line x1="${pl}" y1="${y}" x2="${W-pr}" y2="${y}" stroke="#1c2632"/>`+
          `<text x="${pl-6}" y="${y+3}" text-anchor="end" fill="#5b6675" font-size="10">${fmtN(v)}</text>`;
  }
  days.forEach((d,i)=>{
    const h=Math.max(d.tokens/max*ih,1), x=pl+i*bw, y=pt+ih-h;
    bars+=`<rect x="${x+bw*0.16}" y="${y}" width="${bw*0.68}" height="${h}" rx="2" fill="url(#bg)">
      <title>${d.date}　${fmtN(d.tokens)} tokens　${d.messages} 則　(${fmtUSD(d.cost)} 等值)</title></rect>`;
...
// ── 各模型 donut ──
(function(){
  const m=D.by_model; const total=m.reduce((s,x)=>s+x.tokens,0)||1;
  const colors=['#ff8c42','#5aa9e6','#9b8cff','#3fb68b','#ff6b9d','#f5c451'];
  const cx=78,cy=78,r=58,sw=22; let ang=-Math.PI/2, segs='';
  m.forEach((x,i)=>{
    const frac=x.tokens/total, a2=ang+frac*Math.PI*2;
    const x1=cx+r*Math.cos(ang), y1=cy+r*Math.sin(ang);
    const x2=cx+r*Math.cos(a2), y2=cy+r*Math.sin(a2);
    const large=frac>0.5?1:0;
    segs+=`<path d="M ${x1} ${y1} A ${r} ${r} 0 ${large} 1 ${x2} ${y2}" stroke="${colors[i%6]}" stroke-width="${sw}" fill="none" stroke-linecap="butt"><title>${x.model}　${fmtN(x.tokens)} tokens（${(frac*100).toFixed(1)}%）· ${fmtUSD(x.cost)} 等值</title></path>`;
    ang=a2;
  });
  const leg=m.map((x,i)=>`<span><i style="background:${colors[i%6]}"></i>${esc(x.model)} ${(x.tokens/total*100).toFixed(0)}%</span>`).join('');
  $('modelChart').innerHTML=`
    <div style="display:flex;gap:14px;align-items:center">
      <svg viewBox="0 0 156 156" style="width:150px;flex:none">${segs}
        <text x="78" y="74" text-anchor="middle" fill="#e6edf3" font-size="19" font-weight="700">${fmtN(total)}</text>
        <text x="78" y="92" text-anchor="middle" fill="#8b98a9" font-size="10">總 token</text></svg>
...
// ── 專案排行 ──
(function(){
  const p=D.by_project.slice(0,8); const max=Math.max(...p.map(x=>x.tokens),1);
  const colors=['#ff8c42','#5aa9e6','#9b8cff','#3fb68b','#ff6b9d','#f5c451','#ffb37a','#6fd3b8'];
  $('projRank').innerHTML=`<div class="rank">`+p.map((x,i)=>`
    <div class="it"><div class="nm"><b>${esc(x.project)}</b><span>${fmtN(x.tokens)} · ${x.messages} 則</span></div>
    <div class="tr"><div class="fl" style="width:${Math.max(x.tokens/max*100,2)}%;background:${colors[i%8]}"></div></div></div>`).join('')+`</div>`;
})();
